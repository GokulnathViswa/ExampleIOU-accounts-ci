 ...
 **/
@StartableByRPC
class GetAccountFromKey(val key:String) :
    ...
    override fun call(): StateAndRef<AccountInfo> {
        ...
        var id = serviceHub.identityService.externalIdForPublicKey(pubKey)
        var account=serviceHub.vaultService.queryBy<AccountInfo>().states.filter{ it.state.data.identifier.id==id}.single()
        return account
